...
func LoadClusterState(filePath string) (map[string]*ClusterStatus, error) {
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		logrus.Infof("Did not find existing Cluster state file at %s. Creating new Cluster state file and returning new empty map[string]*ClusterStatus\n", filePath)
...
func DestroyClusterState(filePath string) error {
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		logrus.Infof("Did not find existing Cluster state file at %s.\n", filePath)
		return nil
	} else if err != nil {
...
func FindOrCreateStatusByName(statuses map[string]*ClusterStatus, name string) *ClusterStatus {
	...
	if clusterStatus == nil {
		logrus.Infof("Did not find existing ClusterStatus object for Cluster with name %s.\n", name)

		newClusterStatus := ClusterStatus{
			Name: name,
		}
		statuses[name] = &newClusterStatus
		logrus.Debugf("Created new ClusterStatus object: %v", statuses)

		return statuses[name]
	}

	logrus.Infof("Found ClusterStatus object named %s", name)
